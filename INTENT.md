...
- **2026-05-10** (agent dogfooding 첫 *성공*) — TODO(human): Operator 본인이 본 entry를 직접 작성. context: 세션 재시작 후 `experiment-designer` agent가 첫 호출에서 정상 작동, G1 첫 smoke plan(`docs/superpowers/plans/2026-05-10-g1-first-smoke.md`, freeze tag `g1-smoke-pre` @ commit `2be69ed`)을 message-only로 산출, Operator가 권장 5/5 수용 후 commit. Learnings #2의 *negative* path("agent type not found")가 이번 turn의 *positive* path로 같은 invariant를 양방향에서 검증함. 본 entry에 (a) plan 품질 중 가장 인상 깊었던 1-2 항목 (Not declaration table? §5.4 인용? KG-A determinism 추가 criterion? smoke level 자각?), (b) Operator로서 검토 시 마찰점이 있었는지(없으면 그것도 evidence), (c) 의도공학 layer가 *agent 단위*에서 작동했다는 1차 사례로서의 의미를 5-10줄로 기록.
