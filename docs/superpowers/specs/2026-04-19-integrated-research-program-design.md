...
```
 ...
 │   - Nix bundle: LibreLane 3.0.2 + OpenROAD + Yosys + sky130A      │
...
TOOL plane (SHA-pinned)
 └ LibreLane 3.0.2 (FOSSi Foundation) + OpenROAD + Yosys + Verilator + cocotb
...
### 6.2 Reproducibility Lockfile (Codex M8 대응, K2 ζ 2026-04-22 갱신)

SHA pinning 이상으로 재현성을 못박기 위한 단일 lockfile 의무화.

- **파일 위치**: 리포 루트 `lockfile.yaml`
- **필수 필드**:
  - `commit_shas`: OpenROAD, LibreLane, Yosys, Verilator, cocotb, Chipyard, Gemmini, mlcommons/tiny 등 모든 upstream repo의 사용 SHA. K2 ζ 검증(2026-04-22): Chipyard 1.13.0 (2024-09-30 이후 tag 없음) · Gemmini (2023-05 이후 tag 없음) 둘 다 **SHA pin 필수**. LibreLane은 3.0.2 (2026-04-02) tag를 기본 target으로 사용하되 `commit_shas`에 명시.
  ...
  - `pdk_digests`: open_pdks (sky130A + 선택 gf180mcuD) 고정 버전. K2 ζ 노트: 2026-01 IonQ-SkyWater 인수로 sky130A 장기 provenance 리스크 발생 — `source_tarball_mirrors` 우선순위 상향 필요.
...
- **Fargate runtime 전제 (K2 ζ)**: Fargate Task는 ephemeral storage를 최대 **200 GiB**까지 설정 가능(기본 20 GiB, 이전 spec이 가정한 10 GiB 제약은 오류) — LibreLane 3.0.2 + Chipyard + Gemmini 통합 Nix image + 중간 산출물을 한 컨테이너에서 처리할 여유 확보.
- **Step Functions workflow type (K2 ζ)**: **Standard workflow**를 기본값으로 사용. Express는 5분 실행 상한이 있어 20분 넘는 PnR job은 중단됨. Standard는 최대 1년 실행 가능해 `run-task`·`wait-for-completion` 패턴에 적합. Distributed Map maxConcurrency는 기본 1,000 · 상한 10,000이며 본 프로그램의 `maxConcurrency=10`은 안전 범위.
...
| EDA 스택 | `efabless/openlane2@main` (Nix) | **LibreLane 3.0.2** (FOSSi Foundation, 2026-04-02 tag) — OpenLane2 → LibreLane 이름·오너십 변경(K1, 2025-08) + K2 ζ 2026-04-22 재검증 시 2.4 → 3.0.2로 추가 업데이트 |
...
- **Kill gates (각 fail 시 L1 재설계 강제, L2/L3 진입 불가)**:
  - **KG-A (LibreLane on Fargate)**: LibreLane 3.0.2 (2026-04-02 tag, FOSSi Foundation) Nix 이미지가 Fargate Spot 4vCPU 16GB · ephemeral storage 200 GiB 에서 gcd flow를 timeout(30분) 내 완주. 실패 시 8vCPU/32GB 또는 OpenROAD-only fallback 검토. (K2 ζ: LibreLane 2.4 → 3.0.2 버전 갱신, Fargate ephemeral 10 GB → 200 GiB로 전제 수정 완료)
...
| LibreLane 3.0.2 Nix가 Fargate에서 미가동 | KG-A에서 조기 검증. 실패 시 OpenROAD 단독 + 수동 스크립트로 다운그레이드 (LibreLane 제거, 성능 penalty 문서화) |
| **Chipyard + Gemmini 빌드 Fargate 메모리 초과** | KG-B. S3 prebuilt cache, 또는 Chipyard 사전 빌드 baseline artifact. 8vCPU/32GB fallback 혼합. K2 ζ: Chipyard 1.13.0 (2024-09-30 이후 tag 없음) · Gemmini (2023-05 이후 tag 없음) 둘 다 SHA pin 필수 — `lockfile.yaml commit_shas`에 반드시 고정 |
...
| **긴 PnR(>20분) 중 Spot reclaim** | KG-D. Pre-emption signal 감지 후 checkpoint 저장, 다음 task에서 resume. 재시도 실패 시 on-demand fallback. K2 ζ: Step Functions는 **Standard workflow**를 사용해야 함 (Express 5분 상한은 20분 PnR 중단시킴 — §6.2) |
...
- **LibreLane 3.0.2**: 공식 config schema에 정의된 모든 step parameter
