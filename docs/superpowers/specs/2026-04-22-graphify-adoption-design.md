...
- CLI: `graphify <path>`, `graphify query "..."`, `graphify install --platform claude-code`.
- MCP 서버: `python -m graphify.serve graph.json` — L2.memory.recall() 구현 후보.
- `--wiki` 플래그: Leiden 커뮤니티별 마크다운 아티클 자동 생성.
...
**실행**
```bash
# S1에서는 glib(isolated) 설치로 빠르게 평가. S3에서 정식 dependency로 편입.
uv tool install graphify
graphify wiki/raw/    --out tmp/graphify-eval/corpus-narrative/
graphify wiki/program/ --out tmp/graphify-eval/corpus-contract/
graphify src/          --out tmp/graphify-eval/corpus-code/
graphify . --exclude node_modules,.venv --out tmp/graphify-eval/corpus-all/
```

`tmp/graphify-eval/`는 `.gitignore`에 추가. 평가 판정만 spec으로 커밋.
...
- `Makefile` 신규 타겟: `make graph` (`graphify . --out build/graph/`), `make graph-serve` (MCP serve), `make graph-lint` (graph integrity check 실행).
