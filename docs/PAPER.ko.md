...
타이밍-슬랙 surrogate 학습에 **처음** 적용하고, **비전문가 operator**가 per-winner 승인 없이 *방향
만으로* 8세대를 진행하게 했다. 핵심 결과는 더 나은 모델이 아니라 **정직한 negative result**다:
in-loop 검증 오차(`val_mae`)는 세대마다 역대 최저를 경신했지만(gen-007 1.29 → gen-008 0.53 ns),
교차설계 일반화는 다섯 세대 내내 baseline과 *통계적으로 구별 불가*였다. 즉 *in-distribution
최적화와 미관측 설계 일반화는 구조적으로 분리*된다—이는 ML-for-EDA에서 이미 문서화된 현상이다.
따라서 우리의 기여는 **프로세스·접근성** 축에 있다: (1) 자율 루프가 사람 설계 없이 이 알려진 벽을
*재현*했고, (2) 4단 **권력분립 게이트**(median → leave-one-design-out → 교차설계 통계 검정 → 독립
코드 리뷰)가 5건의 위양성 승격을 차단하고 baseline 무결성을 지켰으며, (3) 비전문가가 문서화된
...
1. **AutoResearch를 EDA surrogate 학습에 첫 적용**(단일 `train.py` 변형, 고정 예산, population +
   median 선택).
2. **4단 권력분립 게이트**(median → LODO → 교차설계 T1 → 독립 Codex 리뷰)가 **5건**의 위양성 승격
   (gen-002~008)을 차단하고 세대를 거치며 *스스로 진화*.
3. **정직한 negative result**: "in-loop `val_mae` ↓ ≠ 교차설계 일반화"를 자율 루프가 다섯 세대에
   걸쳐 재현하고, 이를 새 발견이 아니라 *알려진* ML-for-EDA 현상으로 명시적으로 위치시킴.
...
**Operator-in-loop:** operator는 개별 winner를 승인하지 않는다; 방향(`program.md`)을 정하고 각 세대를
튜토리얼식 리포트로 따라간다.
...
**벽.** gen-004~008에서 in-loop median `val_mae`는 단조 감소해 역대 최저를 경신했으나, 교차설계 T1
verdict는 내내 `indistinguishable`이었다. 핵심 그림(저장소 `tutorial/assets/the-wall.svg`)이
하강하는 in-loop 곡선과 평탄한 교차설계 곡선을 대비한다.

## 6. 논의

**구조적 분리.** in-distribution 검증 오차를 낮추는 것과 미관측 설계로 일반화하는 것은 *다른
능력*이다. 이는 다섯 세대, 세 가지 선택 지렛대(데이터 추가·힌트 강화·4번째 설계), 두 에이전트
계열에 걸쳐 성립했다.

**게이트의 가치 = 위양성 차단.** median만 믿었다면 역대 최저지만-일반화-안-되는 winner 5개가
baseline을 오염시켰을 것이다. 게이트는 승격할 때가 아니라, 낙관적 in-loop 숫자에 속기를 거부할 때
...
보고한 것을 재진술한다. novelty는 *사람이 설계하지 않은* 루프가 이 벽에 수렴했고 게이트가 이를
포착했다는 점이다. 기여는 발견이 아니라 재현-플러스-포착이다.
