...
def add_timing_features(X: np.ndarray) -> np.ndarray:
    ...
    mean_over_arrival = mean_delay / (np.abs(arrival) + eps)
    criticality = arrival - synth_slack
    abs_slack = np.abs(synth_slack)
    slack_to_arrival = synth_slack / (np.abs(arrival) + eps)
    ff_pair = start_ff * end_ff
    ff_xor = np.abs(start_ff - end_ff)
    boundary_ff_count = start_ff + end_ff
    path_mod2 = np.mod(path_group, 2.0)
    path_mod3 = np.mod(path_group, 3.0)
    ...
    extra = np.column_stack([
        ...
        stages * max_delay,
        stages * delay_spread,
        np.log1p(np.abs(synth_slack)),
        np.log1p(np.abs(arrival)),
        np.log1p(np.abs(total_mean_delay)),
        path_mod2,
        path_mod3,
    ])
...
class SlackBlendRegressor(BaseEstimator, RegressorMixin):
    def __init__(self, absolute_model=None, residual_model=None, residual_weight: float = 0.30):
    ...
    def predict(self, X):
        ...
        w = float(self.residual_weight)
        return (1.0 - w) * abs_pred + w * residual_pred
...
def make_tree_ensemble(seed: int = 0) -> VotingRegressor:
...
def make_model(seed: int = 0) -> Pipeline:
    absolute = make_tree_ensemble(seed)
    ...
    blended = SlackBlendRegressor(
        absolute_model=absolute,
