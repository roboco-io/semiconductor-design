...
def add_timing_features(X: np.ndarray) -> np.ndarray:
    ...
    max_delay_per_stage = max_delay / safe_stages
    mean_to_arrival = mean_delay / (np.abs(arrival) + eps)
    max_to_arrival = max_delay / (np.abs(arrival) + eps)
...
def make_model(seed: int = 0) -> Pipeline:
    ...
    smooth_hgb = HistGradientBoostingRegressor(
        learning_rate=0.04,
        max_iter=360,
        max_leaf_nodes=15,
    ...
    ensemble = VotingRegressor(
        estimators=[
            ("hgb", hgb),
            ("smooth_hgb", smooth_hgb),
            ("et", et),
        ],
        weights=[0.50, 0.18, 0.32],
        n_jobs=-1,
    )
    return Pipeline([
