...
## V4 추가 실측 (2026-06-11, post-probe 옵션 A — V1×V3 조합)

| 스크립트 | aes (held-out) | gcd (held-out) | verdict |
|---|---|---|---|
| naive | 1.7198 | 1.4117 | (기준) |
| v4_delta_ratio | 1.2949 | 3.8882 | partial |

- **V4(델타+비율)는 V1 단독보다 양쪽 모두 나쁨**(aes 1.29 vs 1.08, gcd 3.89 vs 1.57).
  무차원 비율 feature가 델타 label과 결합해도 도움이 안 되고, 특히 gcd held-out에서 크게 악화 —
  비율화가 잃는 절대 스케일 정보가 델타 학습에도 필요함을 시사.
- **축 결론 강화: V1(델타 label 단독)이 현 시점 최선의 교차설계 구성.** 조합 탐색은 여기서 종료
  (2-fold 다중비교 과적합 위험 — 브리프 옵션 A의 사전 경고대로 더 쥐어짜지 않음).
