...
def add_timing_features(X: np.ndarray) -> np.ndarray:
    ...
    eps = 1e-9
    stages_safe = np.maximum(stages, 1.0)
    total_mean_delay = stages * mean_delay
    delay_spread = max_delay - mean_delay
    delay_ratio = max_delay / (np.abs(mean_delay) + eps)
    slack_per_stage = synth_slack / (stages_safe + eps)
    arrival_per_stage = arrival / (stages_safe + eps)
    criticality = arrival - synth_slack
    delay_density = total_mean_delay / (arrival + np.sign(arrival) * eps + eps)
...
def make_fallback_model(seed: int = 0) -> Pipeline:
    et = ExtraTreesRegressor(
        n_estimators=520,
        max_features=0.9,
        min_samples_leaf=1,
...
def train_and_eval(X, y, groups, seed: int = 0) -> tuple[object, float]:
    ...
    models = [make_model(seed), make_fallback_model(seed)]
    best_model = None
    best_mae = float("inf")
