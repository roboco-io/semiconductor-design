전략: conservative — baseline 구조(HGB+ExtraTrees Voting + timing features)를 유지하고 정규화·일반화 방향으로 소폭만 튜닝했습니다 (관찰 힌트의 LODO 후퇴 신호 반영: in-loop val 과적합 완화).
...
def make_model(seed: int = 0) -> Pipeline:
    # conservative: baseline 구조 유지 + 정규화 소폭 강화(미관측 설계 일반화 후퇴 완화 목적).
