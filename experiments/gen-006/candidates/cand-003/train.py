...
def add_timing_features(X: np.ndarray) -> np.ndarray:
    ...
    slack_delay_margin = synth_slack - total_mean_delay
    arrival_delay_ratio = arrival / (np.abs(total_mean_delay) + eps)
...
class SlackAnchorRegressor(BaseEstimator, RegressorMixin):
    def __init__(self, base_estimator, model_weight: float = 0.96):
    ...
    def predict(self, X):
        ...
        return self.model_weight * pred + (1.0 - self.model_weight) * X[:, 1]
...
def make_model(seed: int = 0) -> Pipeline:
    ...
    anchored = SlackAnchorRegressor(ensemble, model_weight=0.96)
    return Pipeline([
