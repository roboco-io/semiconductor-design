...
> 아래는 핵심 산출물 디렉터리만 추린 것이다. **세대별 결과 요약(gen-001~008)은 하단 튜토리얼 시리즈 표**를 보라.

| 디렉터리 | 내용 |
|---|---|
| `real-gcd-fargate/` | 진짜 EDA flow 산출(STA 리포트 + dataset 53행). `VALIDATION.md` 참고 |
| `real-jpeg-fargate/` | 4번째 설계 jpeg ORFS flow 산출(synth/route 리포트 + dataset 4410행, Sub-A) |
| `multidesign/` | 다설계 결합본(`dataset-4design.jsonl` 7194행) + 교차설계 probe 기록 |
| `gen-001/` | 첫 세대 — Codex winner 승격(tag `gen-001-best`, `promoted`). 소급 재심에서 H-A 엄밀 재확증(`revalidation.md`, `distinguishable`, dz=−1.27) |
| `gen-002/` ~ `gen-008/` | 2~8세대. 모두 미승격(reject) — 4단 게이트가 위양성 승격 차단. 각 세대 `README.md`(튜토리얼)·`report.md`(게이트 표) 참고 |
