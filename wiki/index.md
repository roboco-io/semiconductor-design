...
| `raw/docs/` | 공식 문서·종합 가이드 | `c-eda-flow-operator-guide` (perplexity 종합) + `librelane-3-architecture-official` (exa.ai LibreLane 공식) — **2026-05-09 추가** |
| `raw/repos/` | GitHub repo·tool 발견 | `librelane-summary-tool` (mattvenn) — **2026-05-09 추가** |
...
| F branch — PDK 파일 포맷 (`.lib`/`.lef`/`.def`/`.sdc`) | ↑↑ | curriculum elevate, operator lens lingua franca. perplexity → exa 2단 탐색 후 컴파일 |
| OpenSTA 명령 reference (raw/docs 추가) | ↑ | `report_checks` flag, exception path, false_path, multicycle |
