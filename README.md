...
> Karpathy식 **자율 진행이 기본** — 사람(Operator)은 매 winner를 승인하는 게 아니라
> **방향을 잡고 큰 흐름을 이해**한다. 채택은 **객관적 자동 게이트**가 판정.
> 목표: **비전문가가 전문영역(EDA)서 자율 에이전트로 의미있는 성과**를 내는 것.
>
...
- **루프**: 세대마다 N개 후보 변형(Claude+Codex) → 각각 **5개 seed로 학습** → 최저
  **median `val_mae`** winner 선택 → **객관적 자동 게이트(median + T1 검증) 통과 시 자동 승격**.
- **차별 축(가설, 2026-06-08 재피벗)**: **H-A** 에이전트 루프가 사람 수작업을 능가(gen-001 엄밀 확증) ·
  **H-B(재정의)** 비전문가가 *per-winner 승인 없이* 방향·이해만으로 신뢰가능한 자율을 운영 —
  novelty 축이 *Operator authority*에서 **비전문가 empowerment + 이해가능성**으로 이동.
...
substrate(`prepare.py`, 평가 규칙)는 **고정(frozen)** — 공정 비교를 위해. winner 승격은
**객관적 자동 게이트(median + T1)** 가 판정(2026-06-08 재피벗 — per-winner 사람 승인에서 전환 중).
